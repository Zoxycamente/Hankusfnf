...
/**
	A new chart editor. Mainly focused on Psych Engine charting system
**/
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var gfs:Array<String> = CoolUtil.coolTextFile(Paths.txt('data/gfList'));
		...
		daStageDropDown.selectedLabel = _song.curStage;
		...
		tab_group_song.add(playerGFDropDown);
		tab_group_song.add(playTicksBf);
		tab_group_song.add(playTicksDad);
		tab_group_song.add(daStageDropDown);
	...
	private function bopIcons():Void
	{
		FlxTween.cancelTweensOf(leftIcon);
		FlxTween.cancelTweensOf(rightIcon);

		leftIcon.scale.set(0.5, 0.5);
