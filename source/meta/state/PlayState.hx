...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var rencusChange:Character = new Character().setCharacter(dadOpponent.x,dadOpponent.y,'rencus');
        add(rencusChange);
        rencusChange.visible = false;

		var hankusPoligonalChange:Character = new Character().setCharacter(dadOpponent.x,dadOpponent.y,'hankusPoligonal');
        add(hankusPoligonalChange);
        hankusPoligonalChange.visible = false;
		...
		switch(PlayState.SONG.song)
				{
					case 'apotheosis' | 'purple-alert':
					Application.current.window.title = "Friday Night Funkin': V.S Hankus 2023 Remake - " + PlayState.SONG.song + " - Canon song" + " (Universe 0016A)";
					case 'dead-memories':
					Application.current.window.title = "Friday Night Funkin': V.S Hankus 2023 Remake - " + PlayState.SONG.song + " - Uncanon song" + " (Incident 0012A)";
					case 'sussy-legacy':
					Application.current.window.title = "Friday Night Funkin': V.S Hankus 2023 Remake - " + PlayState.SONG.song + " - Uncanon song" + " (Incident 0201B)";
					case 'sabotage-error':
					Application.current.window.title = "Fr1d4y N1ght Funk1n': V.S TH3 D4RKN3$$ - " + "$4b0t4ge &rr0r" + " - Unc4n0n s0ng" + " (Incident 2203A)";
					case 'poligonal-suspicion':
					Application.current.window.title = "Friday Night Funkin': V.S Hankus 2023 Remake - " + PlayState.SONG.song + " - Uncanon song" + " (Incident 3303D)";
				}
		...
		songPosBG = new FlxSprite(0, 10).loadGraphic(Paths.image('UI/barsThing/healthBar'));
		if (!Init.trueSettings.get('Downscroll'))
		songPosBG.y = 10;
		else
		songPosBG.y = FlxG.height * 0.950;
		...
		if (PlayState.curSong == "sussy-legacy") {
          ...
          songPosBar.createFilledBar(FlxColor.fromString('#0F0F0F'), dadOpponent.barColor);
        }
		songPosBar.alpha = 0;
		...
		var Overlay:FlxSprite = new FlxSprite(0).loadGraphic(Paths.image('UI/barsThing/Overlay'));
		Overlay.screenCenter(X);
		Overlay.scrollFactor.set();
		Overlay.alpha = 0.5;
		Overlay.cameras = [camHUD];
		if (Init.trueSettings.get("Overlay"))
		add(Overlay);

		var OverlayD:FlxSprite = new FlxSprite(0, 705).loadGraphic(Paths.image('UI/barsThing/Overlay'));
		OverlayD.screenCenter(X);
		OverlayD.scrollFactor.set();
		OverlayD.alpha = 0.5;
		OverlayD.cameras = [camHUD];
		if (Init.trueSettings.get("Overlay"))
		add(OverlayD);
		...
		if (Init.trueSettings.get('Downscroll'))
			barY = 64;
		healthBarBG = new FlxSprite(0, barY).loadGraphic(Paths.image('UI/barsThing/healthBar'));
		...
		/*
			var shader:GraphicsShader = new GraphicsShader("", File.getContent("./assets/data/shaders/vhs.frag"));
	...
	public function reloadCharacterIcons() {
	    ...
	    //healthBar.createFilledBar(0xFFFF0000, 0xFF66FF33); me when classic health bar
		healthBar.createFilledBar(dadOpponent.barColor, boyfriend.barColor);
	    //healthBar
		add(healthBar);

		var ObarY = FlxG.height * 0.875;
		if (Init.trueSettings.get('Downscroll'))
			ObarY = 64;
		var healthBarOverlay:FlxSprite = new FlxSprite(0, ObarY).loadGraphic(Paths.image('UI/barsThing/healthBarOverlay'));
		healthBarOverlay.screenCenter(X);
		healthBarOverlay.scrollFactor.set();
		//healthBarOverlay.blend = ADD;
		healthBarOverlay.alpha = 0.5;
		if (Init.trueSettings.get("Healthbar Overlay"))
	...
	function popUpCombo(?cache:Bool = false)
	{
		...
		{
			// numScore.loadGraphic(Paths.image('UI/barsThing/' + pixelModifier + 'num' + stringArray[scoreInt]));
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'sussy-legacy')
			{
				switch (curBeat)
				{ 
					case 1:
					   ...
					   PlayState.boyfriendStrums.visible = false;
					case 16:
					   PlayState.camHUD.visible = true;
					   PlayState.dadStrums.visible = true;
					   PlayState.boyfriendStrums.visible = true;
					   if (!Init.trueSettings.get('Disable Flashing Lights'))
					   FlxG.camera.flash(FlxColor.WHITE, 1);
					case 48 | 80 | 128 | 192 | 208 | 256 | 320 | 352:
						if (!Init.trueSettings.get('Disable Flashing Lights'))
					   FlxG.camera.flash(FlxColor.WHITE, 1);
					case 112 | 160 | 176 | 224 | 240 | 304:
						if (!Init.trueSettings.get('Disable Flashing Lights'))
			...
			if (curSong.toLowerCase() == 'purple-alert')
				{
						switch (curBeat)
					{
					case 1:
					...
					case 16:
						defaultCamZoom = 0.9;
					case 29:
						FlxTween.tween(FlxG.camera, {zoom: 1.1}, 1.8);
					case 32:
						defaultCamZoom += 0.5;
						if (!Init.trueSettings.get('Disable Flashing Lights'))
							FlxG.camera.flash(FlxColor.WHITE, 1);
			...
			if (curSong.toLowerCase() == 'poligonal-suspicion')
			{
				switch (curBeat)
				{
			        case 1:
					...
					case 96:
						...
						FlxG.camera.flash(FlxColor.WHITE, 1);
						fadeWhiteFlash();
					case 98 | 100 | 102 | 104 | 106 | 108 | 110 | 112 | 114 | 116 | 118 | 120 | 122 | 124 | 126 | 128 | 130 | 132 | 134 | 136 | 138 | 140 | 142 | 144 | 146 | 148 | 150 | 152 | 154 | 156 | 158:
					    fadeWhiteFlash();
					case 160:
						dadOpponent.setCharacter(dadOpponent.x, dadOpponent.y - -600, 'rencus');
						if (!Init.trueSettings.get('Disable Flashing Lights'))
							FlxG.camera.flash(FlxColor.BLACK, 1);
					case 224 | 384:
						if (!Init.trueSettings.get('Disable Flashing Lights'))
							FlxG.camera.flash(FlxColor.WHITE, 1);	
					case 352:
						dadOpponent.setCharacter(dadOpponent.x, dadOpponent.y - -900, 'hankusPoligonal');
						if (!Init.trueSettings.get('Disable Flashing Lights'))
							FlxG.camera.flash(FlxColor.WHITE, 1);
					}
				}
			...
			if (curSong.toLowerCase() == 'dead-memories')
			{
				switch (curBeat)
				{
					case 64:
	...
	public function songIntroCutscene()
	{
		switch (curSong.toLowerCase())
		{
			...
			case 'thorns':
				...
				senpaiEvil.frames = Paths.getSparrowAtlas('data/cutscene/senpai/senpaiCrazy');
