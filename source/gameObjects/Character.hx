	...
	public function setCharacter(x:Float, y:Float, character:String):Character
	{
		...
		switch (curCharacter)
		{
			...
			case 'bf':
				...
				if (!Init.trueSettings.get('Classic Health Bar'))
				barColor = 0xFFAD80A1; //0xFFF5DB5A
			case 'bf-shadering':
			...
			case 'bf-dead':
				...
				flipX = true;
			default:
				frames = Paths.getSparrowAtlas('characters/sprites/placehankus');
				...
				addOffset("idle", -130, -320);
				addOffset("singLEFT", 99, -210);
				addOffset("singRIGHT", -145, -289);
				addOffset("singDOWN", -146, -554);
				addOffset("singUP", -57, -126);
		
		        setGraphicSize(Std.int(width * 1.6));
				playAnim('idle');

				if (!Init.trueSettings.get('Classic Health Bar')) //esse codigo é muito feio me desculpa
				barColor = 0xFF272337;
		}
